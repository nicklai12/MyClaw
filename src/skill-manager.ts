...
**只有在以下情況才回傳技能配置 JSON：**
- 使用者明確說「建立技能」「新增技能」「建立一個...技能」
- 使用者明確描述想要創建一個新的自動化任務、定時提醒、或特定觸發條件下的回應行為
- 使用者用命令式語氣要求創建（如「幫我做一個每天提醒我的功能」）

**以下情況請回覆 NOT_SKILL：**
- 閒聊、問問題、一般對話
- **包含「舉例」「範例」「舉個例子」「例如」等詞彙（這是請求舉例說明，不是建立技能）**
- **包含「如何使用」「怎麼用」「用法是什麼」「原理是什麼」（這是詢問用法，不是建立技能）**
- **包含「教學」「說明」「解釋」（這是請求教學，不是建立技能）**
- **詢問「什麼是...」「什麼意思」（這是詢問定義，不是建立技能）**
- 使用者提到某個技能名稱但沒有明確說要建立它（如「自動追蹤爆款 YouTube 影片是什麼？」）
...
- trigger_value：觸發值（keyword 時為關鍵字**必須以 / 開頭**、pattern 時為正則表達式、cron 時為 cron 表達式、manual/always 為空字串）
...
- 提到「當我說...」「當我傳...」等特定關鍵字 → keyword（trigger_value 必須以 / 開頭）
...
**所有 keyword 類型的 trigger_value 必須以 \`/\` 開頭**，這是為了避免在日常對話中誤觸發。

**範例：**
- 使用者說「觸發關鍵字是搜尋」→ 實際儲存 \"trigger_value\": \"/搜尋\"
- 使用者說「關鍵字是時間」→ 實際儲存 \"trigger_value\": \"/時間\"
- 使用者說「用 /db 觸發」→ 實際儲存 \"trigger_value\": \"/db\"

**如果使用者沒有提供 / 前綴，請自動為 keyword 類型的觸發值加上 / 前綴。**
...
export async function parseSkillFromText(
  ...
  try {
    ...
    if (askingPatterns.some(pattern => text.includes(pattern))) {
      console.log('[skill-manager] 偵測到詢問/舉例意圖，跳過技能建立判斷');
      return null;
    }
    ...
    // 確保 keyword 類型的 trigger_value 有 / 前綴
    let triggerValue = typeof parsed.trigger_value === 'string' ? parsed.trigger_value : '';
    if (triggerType === 'keyword' && triggerValue && !triggerValue.startsWith('/')) {
