...
|------|------|
| 多階段建構 | 編譯與執行分離，映像檔更小 |
| 非 root 執行 | 使用 `appuser` 使用者運行，提升安全性 |
| 健康檢查 | 每 30 秒檢查 `/health` 端點 |
| 資源限制 | 限制 1 CPU / 512MB 記憶體 |
| 日誌輪替 | 單檔 10MB，保留 3 個歷史檔案 |
| 自動重啟 | 除非手動停止，否則自動重啟 |
...
| 觸發類型 | 說明 | 範例 |
|----------|------|------|
| `keyword` | **前綴觸發** — trigger_value 必須以 `/` 開頭，訊息開頭匹配才觸發 | `/搜尋`、`/時間`、`/db` |
| `pattern` | 正則表達式匹配（用於複雜格式，如 URL、電話號碼）| `https?://.+` |
| `cron` | 定時排程觸發（使用 cron 表達式）| `0 9 * * *`（每天早上9點）|
| `manual` | 手動觸發（不透過訊息自動觸發）| — |
| `always` | 總是觸發（當無其他技能匹配時作為預設回應）| — |

### Keyword 觸發規範

**所有 keyword 類型的 trigger_value 必須以 `/` 開頭**，例如：
- ✅ `/搜尋` — 有效，訊息以「/搜尋」開頭時觸發
- ❌ `搜尋` — 無效，不會觸發任何技能

這個設計避免在日常對話中誤觸發技能，讓使用意圖更明確。
...
```
建立技能：資料故事敘述，觸發關鍵字是「/報告」，功能是將數據轉化為引人入勝的故事，使用視覺化、情境和說服性結構來呈現分析結果，來源是 https://github.com/wshobson/agents/tree/main/plugins/business-analytics/skills/data-storytelling
```

**測試（Skill Chaining — 同時觸發「/總結」+「/報告」兩個技能）：**

```
/總結 https://www.bbc.com/news 做成一份 /報告
```
...
```
建立技能：前端設計，觸發關鍵字是「/網頁」，功能是創建獨特且高品質的網頁介面，生成具有創意且精緻的程式碼，來源是 https://github.com/anthropics/claude-plugins-official/blob/main/plugins/frontend-design/skills/frontend-design/SKILL.md
```
...
- [開發環境設定架構](research/25-development-setup/README.md) - devcontainer、Dockerfile、docker-compose 的分工說明

## GitHub Actions

自動化建置 Docker 映像檔並推送到 Docker Hub。

### 設定方式

1. 在 Docker Hub 建立 [Access Token](https://hub.docker.com/settings/security)
2. 在 GitHub Repository → Settings → Secrets and variables → Actions 中新增：
...
- Push 到 `main` 分支時自動觸發
- 支援多平台建置：`linux/amd64`, `linux/arm64`
- 標籤：`latest` 和 commit SHA（短格式）

### 手動觸發

如需手動觸發建置：

1. 到 GitHub Repository → Actions → Build and Push Docker Image
2. 點擊 "Run workflow" → 選擇 `main` 分支
